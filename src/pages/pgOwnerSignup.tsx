...
import { User, Home, Plus, Check, Loader, DollarSign, Image, X, MapPin, Video, Camera, Star, FileText } from 'lucide-react';
...
export function PGOwnerSignup() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await salesPost('/pg-owner/public-signup', data);
      alert('PG Owner Registration Submitted Successfully! Our team will contact you shortly.');
      navigate('/');
    } catch (error: any) {
      console.error('Error:', error);
      alert(error.response?.data?.message || 'Error submitting registration');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={getContainerStyle(isMobile)}>
      ...
      <div style={{ ...getCardStyle(isMobile), display: 'flex', alignItems: 'flex-start', gap: '12px', maxWidth: '900px' }}>
        <input
          type="checkbox"
          id="terms-checkbox"
        ...
        />
        <label htmlFor="terms-checkbox" style={{ fontSize: isMobile ? '13px' : '14px', color: '#555', cursor: 'pointer', lineHeight: 1.5 }}>
          <FileText size={16} style={{ verticalAlign: 'middle', marginRight: '4px', color: '#667eea' }} />
          I agree to the{' '}
          <a
            href="https://www.manageyourpg.com/terms"
            target="_blank"
      ...
      <button style={{ ...submitBtnStyle, opacity: (loading || !termsAccepted) ? 0.5 : 1, maxWidth: '900px', margin: '0 auto' }} onClick={handleSubmit} disabled={loading || !termsAccepted}>
